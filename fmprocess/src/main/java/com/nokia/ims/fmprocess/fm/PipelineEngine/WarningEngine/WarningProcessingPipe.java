package com.nokia.ims.fmprocess.fm.PipelineEngine.WarningEngine;
...
/*This Stage is used for alarm processing operations like filtering and clearing for warning alarms*/
public class WarningProcessingPipe extends CustomPipelineStage {

	static Logger logger = LogManager.getLogger(WarningProcessingPipe.class.getName());
			...
			try {
				...
				if (alarm.EVENT_SEVERITY != 6) {
					if (!alarmFilterEngine.eventFilterCheck(alarm)) {
							...
							FmContext.getWarningConverterRef().write(alarm);
					...
					for (FmEvent event : listofClearAlarms) {
						...
						FmContext.getWarningConverterRef().write(event);
		...
		try {
			...
			for (String searchQuery : searchQueryList) {
				if (searchQuery != null && !searchQuery.isEmpty()) {
					...
					searchQueryArray[0] = searchQueryArray[0] + " AND eventSeverity=4";	
