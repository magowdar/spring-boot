...
/*This Stage is used for alarm processing operations like filtering and clearing for minor alarms*/
public class MinorProcessingPipe extends CustomPipelineStage {

	static Logger logger = LogManager.getLogger(MinorProcessingPipe.class.getName());
			...
			try {
				...
				if (alarm.EVENT_SEVERITY != 6) {
					if (!alarmFilterEngine.eventFilterCheck(alarm)) {
							...
							FmContext.getMinorConverterRef().write(alarm);
					...
					for (FmEvent event : listofClearAlarms) {
						...
						FmContext.getMinorConverterRef().write(event);
		...
		try {
			...
			for (String searchQuery : searchQueryList) {
				if (searchQuery != null && !searchQuery.isEmpty()) {
					...
					searchQueryArray[0]=searchQueryArray[0] + " AND eventSeverity=3";
